...
const PROMPT = `日本人学習者向けに、渡された各英単語について例文を生成してください。

各単語について:
- example_en / example_ja: メインの例文1つ (最も実用的で、日常で一番遭遇しそうな使われ方)
- extras: 追加2〜3個、register を多様化

**共通ルール — 日常で実際にあり得るシーンのみ:**
- 職場メール/Slack、友人との会話、家族、旅行、ショッピング、ニュース、家事、趣味、SNS など
- register 区分:
    ...
    try {
      const result = await anthropic.messages.parse({
        ...
        messages: [
          {
            role: "user",
            content: `以下 ${batch.length} 単語それぞれに例文 (メイン + 追加2-3個) を返してください:\n\n${list}`,
          },
        ],
      ...
      for (let j = 0; j < batch.length; j++) {
        ...
        const update: Record<string, unknown> = {
          extra_examples: group.extras.length > 0 ? group.extras : null,
        };
        if (force) {
          update.example_en = group.example_en || null;
          update.example_ja = group.example_ja || null;
        }
