        ...
        <div className="max-w-xl mx-auto flex items-center gap-2.5 h-11 px-4">
          ...
          {isTypingCard ? (
            ...
            <>
              ...
              <button
                type="button"
                onClick={() => !forceCloze && setCloze((v) => !v)}
                aria-pressed={cloze}
                disabled={forceCloze}
                className={`h-6 px-2 rounded-full text-[10px] font-semibold transition active:scale-95 disabled:cursor-default ${
                  cloze
                    ? "bg-accent text-accent-foreground"
                    : "bg-surface-2 text-muted"
                }`}
