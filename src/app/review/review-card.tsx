...
export function ReviewCard({ card, remaining }: { card: Card; remaining: number }) {
  ...
  // Auto-play once on reveal: hearing the word right when the meaning lands
  // anchors the audio shape to the just-retrieved meaning. Stays a no-op
  // when audio_url is missing.
  useEffect(() => {
    if (!revealed) return;
        ...
        {card.audio_url && (
          <>
            ...
            <button
              type="button"
              onClick={replayAudio}
              className="text-xs px-3 py-1 rounded-full border border-zinc-300 dark:border-zinc-700 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-900"
              aria-label="発音を再生"
            >
              ▶︎ 発音
            </button>
